...
const AlunoListar = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:3005/professores').then(resposta => {
            setObjetos(resposta.data);
        });
    }, []);

    const excluir = id => {
        axios.delete(`http://localhost:3005/professores/${id}`).then(resposta => {
    ...
    return (
        <>
            <Link to="/professores-inserir" className="btn btn-primary">Inserir</Link>
